...
static char *email_notify_note = NULL;
          ...
          if (file != NULL) {
            ...
            if (email_notify != NULL) {
              ...
              if (file != NULL) {
                ...
                if(email_notify_note !=NULL)
                {
                fprintf(file, "Note %s\n\n", email_notify_note);
                }
                fprintf(file, "HTTP Blacklisted IP: %s\n", r->useragent_ip);
                
                fprintf(file, "Host: %s Uri: %s \n",r->hostname, r->unparsed_uri); 
               
                fprintf(file, "\n-----------------------------\n");
                fprintf(file, "mod_evasive Version PCSAG\n");
...
static const char *
get_email_notify_note(cmd_parms *cmd, void *dconfig, const char *value) {
  if (value != NULL && value[0] != 0) {
    if (email_notify_note != NULL)
      free(email_notify_note);
    email_notify_note = strdup(value);
  }
...
static const command_rec access_cmds[] =
{
  ...
  AP_INIT_TAKE1("DOSEmailNotifyNote", get_email_notify_note, NULL, RSRC_CONF,
		"Set email notification note"),
